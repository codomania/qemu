...
struct KVMState
{
    ...
    void (*memcrypt_set_memory_region)(void *handle, MemoryRegion *mr);
};
...
{
    if (kvm_state->memcrypt_handle &&
        kvm_state->memcrypt_set_memory_region) {
        kvm_state->memcrypt_set_memory_region(kvm_state->memcrypt_handle, mr);
    }
}
